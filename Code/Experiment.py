...
def evaluate_algorithm(algo_name, X_scaled, y_true, valid_k_values, h_values, sigma_values, queue):
    best_auc = -1
    best_fpr = None
    best_tpr = None
    ...
    try:
        ...
        queue.put((best_auc, best_fpr, best_tpr))
    except Exception:
...
def main():
    ...
    for i, file_name in enumerate(mat_files):
        print(f"\nProcessing ({i+1}/{n_datasets}): {file_name}")
        ...
        for algo_idx, algo_name in enumerate(algorithms):
            ...
            q = ctx.Queue()
            p = ctx.Process(target=evaluate_algorithm, args=(algo_name, X_scaled, y_true, valid_k_values, h_values, sigma_values, q))
            
            p.start()
            p.join(timeout=TIMEOUT_SECONDS)

            if p.is_alive():
                print(f"  [Timeout!] Exceeded {TIMEOUT_SECONDS}s, terminated.")
                p.terminate()
                p.join()
                best_auc = -1
                ...
                if not q.empty():
                    best_auc, best_fpr, best_tpr = q.get()
                    print(f"  Done. (AUC: {best_auc:.3f})" if best_auc != -1 else "  [Error]")
                else:
